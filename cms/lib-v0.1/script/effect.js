...
/*********************************/
$(document).ready(function() {
	var slideNext = function(container, itemWidth, totalWidth) {
		var leftPos = parseInt($(container).css('left')) - itemWidth;
		if(leftPos + totalWidth < 0) {
			$(container).css({'left': 0});
			leftPos = -itemWidth;
		}
		$(container).animate({'left': leftPos}, 500);
	};
	
	var containers = $('ul.rotate-hori');
	var itemIndex = 1;
	containers.each(function(i, container) {
		$(container).css({position: 'relative', width: '18000px', left: '0px'});
		...
		itemsClone.each(function(j, item) {
			$(item).css({float: 'left'});
		});
		...
		var maskWidth = $(container).attr('maskWidth');
		var mask = $("<div class='mask'></div>");
		mask.css({'position': 'relative', 'overflow': 'hidden', width: maskWidth, height: '400'});
		$(container).wrap(mask);
		
		itemsClone.appendTo(container);
		
		var autoSlide;
		autoSlide = setInterval(function() { slideNext(container, itemWidth, totalWidth) }, 1000);
	});
});